...
class DepthProcessingCNN(nn.Module):
    def __init__(self, input_channels=1, output_dim=512):
        ...
        self.fc = nn.Linear(12544, output_dim)  # Adjust 7*7 based on input size

    def forward(self, depth):
        N, T, H, W = depth.shape
        depth = einops.rearrange(depth, "N T H W -> (N T) 1 H W")
        features = self.conv(depth)
        features = self.flatten(features)
        features = self.fc(features)
        return einops.rearrange(features, "(N T) D -> N T D", N=N)
...
class BehaviorTransformer(nn.Module):
    ...
    def __init__(
        ...
        self._gpt_model = gpt_model.to(device)
        ...
        if visual_input:
            ...
            self._depth_processing = DepthProcessingCNN(input_channels=1, output_dim=512).to(device)
    ...
    def _predict(
        ...
        if self.visual_input:
            ...
            if not (self._cbet_method == self.GOAL_SPEC.unconditional):
                goal_seq = goal_seq.to(self.device)
                if goal_seq.ndim == 3:
        ...
        if self.visual_input:
            if depth_seq.shape[-1] != 224:
                depth_seq = F.interpolate(depth_seq, size=100)
                # now do median and make it even smaller
        ...
        indices = (
            torch.arange(NT).unsqueeze(1).to(self.device),
            torch.arange(self._G).unsqueeze(0).to(self.device),
            sampled_centers,
        )
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas):
        ...
        if self.visual_input:
            ...
            optimizer1.add_param_group({"params": self._depth_processing.parameters()})
    ...
    def save_model(self, path: Path):
        ...
        if hasattr(self, "resnet"):
            ...
            torch.save(self._depth_processing.state_dict(), path / "depth_processing.pt")
    ...
    def load_model(self, path: Path):
        if (path / "cbet_model.pt").exists():
            self.load_state_dict(torch.load(path / "cbet_model.pt",  map_location=torch.device('cpu')))
        elif (path / "gpt_model.pt").exists():
            self._gpt_model.load_state_dict(torch.load(path / "gpt_model.pt" , map_location=torch.device('cpu')))
        elif (path / "resnet.pt").exists():
            self.resnet.load_state_dict(torch.load(path / "resnet.pt"))
        elif (path / "resnet_header.pt").exists():
            self._resnet_header.load_state_dict(torch.load(path / "resnet_header.pt"))
        elif (path / "depth_processing.pt").exists():
            self._depth_processing.load_state_dict(torch.load(path / "depth_processing.pt"))
